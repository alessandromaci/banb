    ...
    <div className="min-h-screen bg-gradient-to-b from-[#3B1EFF] via-[#5B3FFF] to-[#1A0F3D] text-white">
      <div className="max-w-md mx-auto px-6 py-8 space-y-8">
        ...
        <div className="space-y-4 pt-8">
          <h2 className="text-sm text-white/70 font-medium">Destination</h2>
          <PaymentMethodButton
            icon={<Building2 className="w-5 h-5 text-white" />}
            label="Withdraw to IBAN"
            onClick={() => router.push("/withdraw/iban")}
          />
        </div>
        ...
        <div className="space-y-4 pt-8">
          ...
          <p className="text-center text-xs text-white/50">
            Funds will be converted from USDC and sent via SEPA transfer.
          </p>
        </div>
      </div>
    </div>
